...
chosen_shape = "random_robot2"     # circle_compl_rand, spiral_compl_rand, logspiral, logcircle, randdisturbed
model_freq = 2

# data_file_pos = data_directory + "data_" + chosen_shape + "_positions_" + str(model_freq) + "Hz.mat"
data_file_pos = data_directory + "positions_" + chosen_shape + "_freq" + str(model_freq) + "_OutsideWater.mat"
...
model_directory = data_directory + chosen_shape + "_model_OutsideWater/"
...
test_shape = "RPS_helix_abovewater"
...
ax1.scatter(line[0, 0], line[0, 1], line[0, 2], c='k', s=200, marker="o")
ax1.scatter(line[-1, 0], line[-1, 1], line[-1, 2], c='k', s=200, marker="*")
...
# ANN_file_name = model_directory + "INV_LSTM_model_weights_" + str(model_freq) + "Hz.h5"
# loaded_model = tf.keras.models.load_model(ANN_file_name)

# Load the frozen trained dynamics model for faster predictions
with gfile.GFile(model_directory + "IDM_dir_" + str(model_freq) + "Hz/INV_LSTM_model_" + Baseline_variant + ".pb", 'rb') as f:
...
for trial_n in range(1, 2):
    new_data = (model_directory + test_shape + "_freq" + str(robot_frequency) + "_" +
                Baseline_variant + "_trial" + str(trial_n) + ".mat")
    ...
    desired_positions.append(next_inst_pos)
    ...
    for l_n in range(1, normalized_line.shape[0]):
        ...
        if np.isnan(predicted_action).any() or np.isnan(next_inst_pos).any():
            print("NaN was seen here: ", predicted_action)
            break
        else:
            if np.any(predicted_action < desired_init_motor_pos) or np.any(predicted_action > max_array):
        ...
        read_motor_n = normalize_it(input_data=read_motor, total_p_max=total_act_max, total_p_min=total_act_min,
