...
# line_ = np.squeeze(line_["collected_positions"])

# nan_indices_line = np.any(np.isnan(line_), axis=1)
# print("Found nan indices: ", nan_indices)

# line = line_[~nan_indices_line]

# randomly choosing points from the end of the array
# last_500 = robot_positions_un[-500:, :]
selected_indices = np.random.choice(robot_positions_un.shape[0], size=20, replace=False)

# Extract the corresponding 3D points
line = robot_positions_un[selected_indices]
        ...
        for itr_n in range(60):
            ...
            if np.isnan(predicted_action).any() or np.isnan(next_inst_pos).any():
                ...
                if np.any(predicted_action < desired_init_motor_pos) or np.any(predicted_action > max_array):
                    print("unsafe action was sent outside the operation limit: ", predicted_action)
